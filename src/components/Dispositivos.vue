      ...
      <v-toolbar flat color="white">
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on }">
            <v-btn color="primary" dark class="mb-2" v-on="on">Nuevo Dispositivo</v-btn>
          </template>
          <v-card>
            <v-card-title>
        ...
        <v-dialog v-model="dialogEliminar" max-width="500px">
          <v-card>
            <v-card-title>
              <span class="headline">Eliminar Dispositivo</span>
            </v-card-title>
            <v-card-text>
              <v-container grid-list-md>
                <v-layout wrap>
                  <v-flex xs12 sm6 md12>
                    <v-card-text
                      >¿Estas seguro que deseas eliminar el dispositivo?</v-card-text
                    >
                  </v-flex>
                  <v-spacer></v-spacer>
...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Nombre Dispositivo", value: "nombre", sortable: true },
      ],
      deleteItem: -1,
      editedIndex: -1,
      editedItem: {
  ...
  methods: {
    ...
    validarGuardado() {
      this.errors = [];
       if (this.nombre === "") {
        this.errors.push("Debe agregar un nombre para el dispositivo");
        return false;
      }
      return true;
    },
