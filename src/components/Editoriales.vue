  ...
  <v-layout align-start>
    <v-flex>
      <v-toolbar flat color="white">
        <v-toolbar-title>Editoriales</v-toolbar-title>
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on }">
            <v-btn color="primary" dark class="mb-2" v-on="on">Nueva Editorial</v-btn>
          </template>
          <v-card>
            <v-card-title>
        ...
        <v-dialog v-model="dialogEliminar" max-width="500px">
          <v-card>
            <v-card-title>
              <span class="headline">Eliminar Editorial</span>
            </v-card-title>
            <v-card-text>
              <v-container grid-list-md>
                <v-layout wrap>
                  <v-flex xs12 sm6 md12>
                    <v-card-text
                      >¿Estas seguro que deseas eliminar la editorial?</v-card-text
                    >
                  </v-flex>
                  <v-spacer></v-spacer>
...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Nombre Editorial", value: "nombre", sortable: true },
      ],
      deleteItem: -1,
      editedIndex: -1,
      editedItem: {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "Nueva Editorial" : "Editar Editorial";
    },
  },
  watch: {
    dialog(val) {
  ...
  methods: {
    ...
    validarGuardado() {
      this.errors = [];
       if (this.nombre === "") {
        this.errors.push("Debe agregar un nombre para la editorial");
        return false;
      }
      return true;
    },
